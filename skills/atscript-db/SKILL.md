...
| Actions              | [actions.md](references/actions.md)                     | `@DbAction` / `@DbActionPK*` / `@DbActionRow*` / `@DbActions*` decorators, row/rows/table actions, `processor: 'backend' \| 'navigate' \| 'custom'`, PK + row resolvers, server-side `disabled` gate, `onDisabledRows`, `ActionDisabledError`, `/meta` `actions[]` shape |
