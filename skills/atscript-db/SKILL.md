...
description: >-
  ...
  and geo search (db.geoPoint, @db.index.geo, geoSearch, $geoWithin, GET /geo).
...
| Schema sync          | [schema-sync.md](references/schema-sync.md)             | FNV-1a hash, `__atscript_control` store, distributed lock (`podId`, `lockTtlMs`, `waitTimeoutMs`), `@db.sync.method`, `safe` mode, sync hooks, `status: 'error'` entries + retry semantics (hash not persisted on error), dropping indexed/view-referenced/fulltext columns, composite index definition drift                                                                                                                                                                                                                                                                                                                      |
