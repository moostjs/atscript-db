...
| Actions              | [actions.md](references/actions.md)                     | `@DbAction` / `@DbActionID*` / `@DbActionRow*` / `@DbActions*` decorators, object-only identifier body (PK or any unique index), row/rows/table actions, `processor: 'backend' \| 'navigate' \| 'custom'`, sync batch `disabled` gate, `onDisabledRows`, `requiredFields`-driven row projection, preferred-id navigate substitution, `ActionDisabledError`, `/meta` `actions[]` shape |
