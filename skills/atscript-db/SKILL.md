...
description: >-
  ...
  REST routes, declarative actions (`@DbAction`/`@DbActionPK*`/`@DbActions*` row/rows/
  table operations surfaced via `/meta`), URL query syntax, browser `Client` (typed CRUD
  + `client.action()`), and `createDbValidatorPlugin()`. DB layer only — for `.as` syntax,
...
| Actions              | [actions.md](references/actions.md)                     | `@DbAction` / `@DbActionPK*` / `@DbActions*` decorators, row/rows/table actions, `processor: 'backend' \| 'navigate' \| 'custom'`, PK-from-body resolvers, `/meta` `actions[]` shape, success-response `{ message? }` convention |
