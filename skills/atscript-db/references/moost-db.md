...
- `fields[<path>]` = `{ filterable, filterOps?, sortable, indexed?, bucketable?, encrypted?, geo?, writeOnly? }`:
  ...
  - `bucketable` (since 0.1.132) — present (`true`) exactly when a calendar bucket over `P` passes the gate: physical capability ∧ no JSON ancestor ∧ `number.timestamp` ∧ (dimension when the table declares dimensions/measures) ∧ adapter has bucket units. Top-level `bucketUnits` (omitted when none). Rejection: `Bucketing field "P" is not permitted — <reason>.` → [calendar-buckets.md](calendar-buckets.md).
