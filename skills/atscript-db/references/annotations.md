...
| `@db.column.filterable`             | Field     | —                                   | Allow field in `/query` filters when the interface is gated `'manual'`. Adapter capability vetoes it regardless of mode — SQL adapters report `filterable: false` on `@db.json` / array fields, Mongo/memory `true`. Since 0.1.128 `/meta` and the gate agree exactly (see moost-db.md § Gate mode).                                                                                     |
