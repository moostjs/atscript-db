...
| `@db.space`                         | Interface | `name: string`                      | Named `DbSpace` binding for multi-database apps (absent → `"default"`). Groups the model in the manifest's `modelsBySpace` and steers `@TableController(Model)` token resolution to `provideDbSpace(space, name)`. See [schema-sync.md § Model manifest](schema-sync.md#model-manifest-never-forget-a-model-in-the-sync-list), [moost-db.md § Binding forms](moost-db.md#binding-forms). |
...
| `@db.column.filterable`             | Field     | —                                   | Allow field in `/query` filters when the interface is gated `'manual'`. Adapter capability vetoes it regardless of mode — SQL adapters report `filterable: false` on `@db.json` / array fields (only `$exists` passes — `filterOps`), Mongo/memory `true`. Since 0.1.128 `/meta` and the gate agree exactly (see moost-db.md § Gate mode).                                               |
| `@db.column.sortable`               | Field     | —                                   | Allow field in `$sort` when the interface is gated `'manual'`; in auto mode every adapter-sortable column is already sortable (the annotation is a no-op there). JSON-stored / array fields are never sortable on any adapter (since 0.1.128 also on Mongo/memory — `$sort` → 400).                                                                                                      |
| `@db.table.filterable`              | Interface | `'auto' \| 'manual'`                | Filter gate. Absent or `'auto'` → every adapter-filterable column. `'manual'` → HTTP 400 (envelope `errors[0].path`) for filters on fields without `@db.column.filterable`. Policy covers filters only — `$groupBy` / `$having` / aggregate fields use physical capability.                                                                                                              |
| `@db.table.sortable`                | Interface | `'auto' \| 'manual'`                | Sort gate. Same semantics for `$sort` keys; `/meta.fields[*].sortable` advertises exactly what the gate accepts (`indexed` is a separate advisory flag).                                                                                                                                                                                                                                 |
| `@db.json`                          | Field     | —                                   | Store as one JSON column instead of flattening. **Has perf cost — activates key remapping.**                                                                                                                                                                                                                                                                                             |
| `@db.ignore`                        | Field     | —                                   | Exclude from DB schema entirely. Applies to view fields too (0.1.128): the field is left out of the generated `CREATE VIEW`, the definition hash and queries. Cannot coexist with `@meta.id`.                                                                                                                                                                                            |
...
| `@db.column.searchable` | none            | Includes a string field in the generic `$search` fallback (escaped case-insensitive substring `$or`) when the adapter has no native TEXT search. Native search wins when configured. A vector-only table counts as having none, so the fallback covers it (since 0.1.131).                    |
