...
| 10  | `/meta`: geo-indexed fields report `geo: true` and `sortable: false` (SQL: also `filterable: false` + `filterOps: ["$exists"]`, plus `"$geoWithin"` only when the adapter is geo-searchable, since 0.1.132); top-level `geoSearchable` is true only when adapter supports geo AND a geo index exists; `crud.geo` lists the endpoint controls. |
