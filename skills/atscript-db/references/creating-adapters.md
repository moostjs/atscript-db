...
| `canFilterField(fd)`               | `fd.storage !== 'json'` | Adapter natively filters into JSON storage (Mongo dot-paths, arrays). Used by `/meta` `fields[*].filterable` AND by the core path guard (`guardPaths`, since 0.1.128) — a `false` rejects the filter with `INVALID_QUERY` before translation. |
