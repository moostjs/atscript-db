...
| 8   | Custom gates / tooling (`@atscript/db`): `resolveCalendarBuckets(controls, fields, aggregate?)` (throws `INVALID_QUERY`); `bucketSourceVerdict(fd, table, adapter)` (since 0.1.133) = the WHOLE bucket-source rule core + moost-db use (rules/order/messages → [calendar-buckets.md](calendar-buckets.md) #6/#8) → `{ ok: true }` \| `{ ok: false, code, reason }` (`reason` = message clause, no period); `table` = `TableMetadata` or `{ jsonValueParents, dimensions, measures }`, `adapter` = adapter or readable. Add HTTP policy (writeOnly) yourself. `isBucketableField(fd)` — deprecated since 0.1.133, use `bucketSourceVerdict`. `isJsonValueField(fd)`, `jsonValueAncestor(path, jsonValueParents)`. |
