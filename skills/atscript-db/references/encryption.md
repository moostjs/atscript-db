...
| 7   | **HTTP returns decrypted values.** Encryption ≠ redaction — gate read access with projections/permissions as usual. `/meta` reports `encrypted: true`, `filterable: false`, `sortable: false`, no `filterOps` (`$exists` rejected too).                                                                        |
