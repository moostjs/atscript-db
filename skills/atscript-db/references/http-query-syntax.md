...
| `$search`      | `$search=quick+brown`                                               | FTS — adapter must support (or the `@db.column.searchable` fallback). Composes with `$groupBy`: rows are narrowed BEFORE grouping, so the rollup and its `$count` cover exactly what the leaf list returns. See [queries.md](queries.md#search-on-an-aggregate-query-since-01130). |
