...
| `$select`      | `$select=id,name,author.name`                                       | Include-list (paths). See § Read-response baseline below. With `$groupBy` also `fn(field)[:alias]` (`count(*)` → key `count_star`) and `bucket(field,unit[,tz][,weekStart])[:alias]` — see § Grouped queries.                                                                              |
| `$skip`        | `$skip=20`                                                          |                                                                                                                                                                                                                                                                                            |
...
| 4   | Labels are `YYYY-MM-DD` strings — quote them in `$having` (`week>='2026-03-01'`).                                                                                                           |
