...
| 6   | Source: the TYPE is the opt-in — `number.timestamp[.created\|.updated]`, stored leaf, not inside a `@db.json`/array column (any adapter), not `@db.encrypted` (`ENC_FIELD_AGG`), not `@db.writeOnly` (HTTP), adapter-filterable, a dimension in strict mode, adapter has bucket units. ONE function applies these in core, HTTP gate and `/meta` — `bucketSourceVerdict` (since 0.1.133); the first failing rule is reported (encrypted → JSON → type → filterable → dimension → units). Custom gates: call it + add your own writeOnly veto → [creating-adapters.md](creating-adapters.md).                                                                                                                                                                                                                                                                  |
...
| 8   | Errors: query-shape rules → `INVALID_QUERY` 400, `path` `$select` / `$groupBy`. Field rules (#6) → `path` = the field, SAME reason clause in both layers: core `Cannot bucket "points" — not a timestamp field (declare it number.timestamp)`, HTTP `Bucketing field "points" is not permitted — <same>.`; other reasons `inside JSON-stored column "meta"`, `adapter cannot filter on this storage type`, `not a dimension` (`INVALID_QUERY`), `adapter has no calendar buckets` (`BUCKET_NOT_SUPPORTED`). Since 0.1.133 in core (≤ 0.1.132: strict-mode source `Field "x" is not a dimension` on `$groupBy`; no units `Calendar bucket "day" is not supported by this adapter` on `$select`). Unit unsupported → `BUCKET_NOT_SUPPORTED` 400, path `$select`. Engine can't resolve zone → `BUCKET_TZ_UNAVAILABLE` **501** — never a silent `null`/UTC label. |
