...
8. DDL failures inside a table (index/FK DDL, `@db.sync.method 'drop'` refused by the engine) become `status: 'error'` entries — never an unhandled throw.
...
1. Schema-level failures NEVER throw — they land on `result.entries` with `status: 'error'` + `errors[]`. Covers: rename conflicts, type changes without `@db.sync.method`, index/FK DDL failures (e.g. `@db.index.unique` over duplicate data), a `@db.sync.method 'drop'` / removed-table drop the engine refused (PG no-CASCADE), a removed table an unmanaged live FK still references. Pre-flight refusals are `error` entries too (`refused: true`) and additionally set `result.status = 'refused'` — nothing at all ran.
