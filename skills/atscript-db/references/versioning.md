...
| 2   | `updateOne({ ...pk })` without `$cas` writes NOTHING: honest existence count `{ 1 \| 0, 0 }`, no bump. `updateMany(filter, {})` → count only, no empty `SET` (batch fence = `touchMany`, below).                                                                      |
...
### Batch touch — `touchMany(keys, { require })` (since 0.1.129)

```ts
await orders.touchMany([
  { id: 1, version: 4 },
  { id: 2, version: 0 },
]); // { 2, 2 }, both + 1
await orders.touchMany(keys, { require: "any" }); // bump what matches, honest counts
```

| #   | Rule                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| --- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | Each key = primary key (composite ok; a unique index does NOT identify a key) + the version column, NOTHING else (`INVALID_QUERY` at `[i].<prop>` otherwise; missing PK field → `[i].<pk>`; missing/non-number version → `[i].version`; duplicate PK → `[i]`; non-versioned table → `""`). `undefined`-valued props are ignored. `keys: []` → `{ 0, 0 }`, no statement.                                                                                               |
| 2   | `require: 'all'` (default): ONE count over the whole `$or` FIRST — short → `CasMismatchError` (`DbError`, `code "CAS_MISMATCH"`, `.matched` / `.expected`, `errors[0].path === "$cas"`, message `touchMany: <n> of <m> rows matched — stale or missing rows`) before any write. Then `updateMany({ $or: pairs }, {})` chunks of ≤ 500 inside ONE `withTransaction`; a summed `matchedCount` short of `keys.length` throws the same error — SQL rolls every bump back. |
| 3   | Memory adapter / Mongo standalone: `withTransaction` is a passthrough — the pre-count covers the common stale case; the post-count race window is accepted (documented).                                                                                                                                                                                                                                                                                              |
| 4   | `require: 'any'`: no pre-count, honest summed result.                                                                                                                                                                                                                                                                                                                                                                                                                 |
| 5   | No `guard`, no `onWrite`, NOT on the REST surface (add your own route; moost-db maps `CAS_MISMATCH` → 409).                                                                                                                                                                                                                                                                                                                                                           |
| 6   | UPGRADE: ≤ 0.1.127 `updateMany(orFilter, {})` bumped versions (some code used it as a batch lock); since 0.1.128 it is a no-op that still reports the full `matchedCount` — it takes NO lock. Use `touchMany`.                                                                                                                                                                                                                                                        |
