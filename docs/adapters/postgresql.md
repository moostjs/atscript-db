...
Since 0.1.128 every drop schema sync owns — removed tables, `@db.sync.method 'drop'`, destructive table-option changes — is a plain `DROP TABLE IF EXISTS` **without `CASCADE`**. When something the run does not drop first still depends on the table — a user-created view, a foreign key from an unmanaged table, or a foreign key from a table that stays in the inventory — PostgreSQL refuses and sync records an `error` entry for that table (the table stays tracked, the hash is withheld, the next run retries) instead of silently dropping the dependents. Put plainly: a surviving table that references a `@db.sync.method 'drop'` table makes that table's recreate an `error` entry on PostgreSQL — drop the constraint or remove the reference first. A removed table that references it is not a blocker: sync drops it right before the recreate (and, likewise, before a primary-key rebuild). The remaining removed tables are dropped children-first; a foreign-key cycle goes in one `DROP TABLE IF EXISTS a, b` statement, which PostgreSQL resolves without `CASCADE`.

Two related behaviours to know:

- **View recreation drops `GRANT`s.** A managed view whose definition changed is `DROP VIEW` + `CREATE VIEW`; roles granted on the old view must be re-granted. The 0.1.128 upgrade recreates every managed view that has joins, a filter or a having clause once. A user-created view depending on a managed view makes that recreate fail with an `error` entry until it is dropped or the managed view is excluded.
- **Identity columns.** `@db.default.increment` is an identity column, not a default: gaining it emits `ALTER COLUMN … SET NOT NULL, ADD GENERATED BY DEFAULT AS IDENTITY`, losing it emits `ALTER COLUMN … DROP IDENTITY IF EXISTS` (earlier releases emitted an invalid `SET DEFAULT increment()` / a rejected `DROP DEFAULT`). A primary-key change on an empty table is one `ALTER TABLE … DROP CONSTRAINT <pk>, ADD PRIMARY KEY (…)`.

`recreateTable` (`@db.sync.method 'recreate'`) still uses `CASCADE` inside its transaction and restores only the inbound foreign keys that pointed at the primary key — unchanged in 0.1.128.
