...
- **`bucketable`** (since 0.1.132) — present (`true`) exactly when a [calendar bucket](/api/calendar-buckets) over the field is accepted — see [which fields can be bucketed](/api/calendar-buckets#which-fields-can-be-bucketed). The top-level `bucketUnits` lists the units that adapter supports.
