...
- **`filterable`** — `true` when value comparisons (equality, ranges, `$in`, `$regex`, …) on the field are accepted: the adapter can compare the column (`canFilterField`) and the field is neither `@db.writeOnly` nor `@db.encrypted`; in `'manual'` mode additionally only for fields carrying `@db.column.filterable`.
- **`filterOps`** (since 0.1.132) — present only when `filterable` is `false` but narrower operators still pass: the list of those operators. On SQL adapters a `@db.json` / array column reports `["$exists"]` and a `db.geoPoint` column `["$exists"]`, plus `"$geoWithin"` when the adapter is geo-searchable. Offer only these operators for the field in a filter UI. The same `@db.writeOnly` / `@db.encrypted` / `'manual'` rules apply, so a field blocked by one of them has no `filterOps` — existence would reveal whether a sealed value is set. See [Existence](/api/queries#existence).
...
- **`bucketable`** (since 0.1.132) — present (`true`) exactly when a [calendar bucket](/api/calendar-buckets) over the field is accepted: a stored `number.timestamp` field outside any JSON column, neither `@db.writeOnly` nor `@db.encrypted`, a dimension when the table declares dimensions or measures, on an adapter with calendar buckets. The top-level `bucketUnits` lists the units that adapter supports.

Adapter capability is a hard gate over the annotation policy: `@db.json` and array columns are never sortable on any adapter (SQL adapters accept only `$exists` on them; MongoDB and the memory adapter filter them natively). Nested-object parents (`contact`) and navigation paths (`assignee`, `assignee.name`) are never listed; on SQL adapters the descendants of a JSON column (`prefs.theme`) are not listed either — they are on MongoDB / memory, where dotted paths are native.

::: tip Parity is enforced (since 0.1.128)
`sortable: true` ⇔ `$sort=<path>` is accepted and `filterable: true` ⇔ a value-comparison filter on `<path>` is accepted — on every adapter, mode and field kind. When `filterable` is `false`, a filter entry is accepted only if its operators are listed in `filterOps` (an `$exists` entry must be its only operator). Everything else, and every path that is not listed, is rejected with HTTP 400 and the envelope `{ message, statusCode: 400, errors: [{ path, message }] }` naming the reason: missing annotation, adapter storage type, `@db.writeOnly`, navigation path (use `$with`), JSON descendant (select the parent), nested object (use one of its leaves). `$groupBy`, `$having` keys and aggregate fields use the physical capability only — the `'manual'` policy applies to filters and `$sort`.
:::
