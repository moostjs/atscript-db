...
| `fields`           | Per-field capability flags (`sortable`, `filterable`, advisory `indexed`, plus `encrypted` / `geo` / `writeOnly` markers, and `filterOps` when only narrower operators such as `$exists` pass — since 0.1.132). Since 0.1.128 they are exact: `sortable: true` ⇔ `$sort` accepted, `filterable: true` ⇔ filter accepted; unlisted paths (nested-object parents, navigation paths, JSON descendants on SQL adapters) are rejected with 400. See [Query Gate](../adapters/annotations#query-gate) |
