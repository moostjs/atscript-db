...
| pure exclusion map (`{ id: 0 }`)             | rewritten to inclusion (all non-ignored own-table fields minus excluded) + every preferred-id field — exclusion CANNOT remove a preferred-id field. Excluding an object parent (`{ address: 0 }`) removes its whole subtree; excluding a leaf keeps the parent's other leaves (since 0.1.134 — before, the parent's leaves came back) |
...
| `fields`           | Per-field capability flags (`sortable`, `filterable`, advisory `indexed`, plus `encrypted` / `geo` / `writeOnly` markers, `filterOps` when only narrower operators such as `$exists` pass, and `bucketable` on fields that accept a calendar bucket — both since 0.1.132). Since 0.1.128 they are exact: `sortable: true` ⇔ `$sort` accepted, `filterable: true` ⇔ filter accepted; unlisted paths (nested-object parents, navigation paths, JSON descendants on SQL adapters) are rejected with 400. See [Query Gate](../adapters/annotations#query-gate) |
...
| `bucketUnits`      | Calendar-bucket units the adapter supports (`["day", "week", "month", "quarter", "year"]`). **Absent** when it has none. Since 0.1.132 — see [Calendar Buckets](/api/calendar-buckets#discovering-support-through-meta)                                                                                                                                                                                                                                                                                                                                    |
...
| `DbError` (BUCKET_TZ_UNAVAILABLE)             | 501         | `{ message, statusCode, errors: [{ path: "$select", message }] }` — the database cannot convert to the bucket's time zone; see [Calendar Buckets — Errors](/api/calendar-buckets#errors) (since 0.1.132) |
...
| `DbError` (other, incl. BUCKET_NOT_SUPPORTED) | 400         | `{ message, statusCode, errors }`                                                                                                                                                                        |
