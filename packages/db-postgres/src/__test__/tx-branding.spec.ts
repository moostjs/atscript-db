 ...
 * core brands transaction state by owner, so PostgreSQL ignores it regardless of
...
class TrapState {
  ...
  private trip(sql: string): never {
    this.used.push(sql);
    throw new Error(`foreign transaction state used as a PostgreSQL connection: ${sql}`);
  }
}
...
describe("PostgresAdapter — transaction state is branded by pool (since 0.1.128)", () => {
  ...
  beforeEach(() => {
    ...
    adapter = new PostgresAdapter(driver);
  ...
  it("a table write inside a foreign-outer transaction opens its own transaction (the table wraps writes), never on the foreign state", async () => {
    ...
    expect(foreign.trap.used).toEqual([]);
    const tx = sqls().filter((s) => /^(BEGIN|COMMIT|ROLLBACK)$/.test(s));
    expect(tx).toEqual(["BEGIN", "COMMIT"]);
    expect(sqls().some((s) => s.startsWith("INSERT"))).toBe(true);
  });

  it("a nested withTransaction inside a foreign-outer context opens its own PostgreSQL transaction on a pool connection", async () => {
    await foreign.withTransaction(async () => {
  ...
  it("two adapters over the same pool join one transaction; a second pool does not", async () => {
    const sibling = new AtscriptDbTable(VersionedItemTable, new PostgresAdapter(driver));
    const otherDriver = createMockDriver({ getResult: { cnt: 1 } });
    const otherPool = new AtscriptDbTable(VersionedItemTable, new PostgresAdapter(otherDriver));
    await adapter.withTransaction(async () => {
      await table.insertOne({ id: 1, name: "a" } as any);
      await sibling.insertOne({ id: 2, name: "b" } as any);
      await otherPool.insertOne({ id: 3, name: "c" } as any);
    });
    const tx = sqls().filter((s) => /^(BEGIN|COMMIT)$/.test(s));
    expect(tx).toEqual(["BEGIN", "COMMIT"]); // one transaction for both tables
    // The other pool is a different owner: its table write opened its OWN transaction there.
    const otherTx = otherDriver.calls.map((c) => c.sql).filter((s) => /^(BEGIN|COMMIT)$/.test(s));
    expect(otherTx).toEqual(["BEGIN", "COMMIT"]);
  });
});
