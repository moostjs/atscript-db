...
// Every prop-level @db.pg.* annotation is structural (native types, collations)
// and must not be inherited by fields referencing the annotated field — a new
// spec that forgets passedWhenReferred: false silently reintroduces the
// ref-inheritance leak for this adapter.
describe("pg annotation tree ref scoping", () => {
  ...
  it("declares every prop-level annotation with passedWhenReferred: false", () => {
    const missing = collectPropSpecs(annotations, "pg")
      .filter(
        ([, spec]) =>
