...
// Every prop-level @db.mongo.* annotation is structural (Atlas Search field
// mappings) and must not be inherited by fields referencing the annotated
// field — a new spec that forgets passedWhenReferred: false silently
// reintroduces the ref-inheritance leak for this adapter.
describe("mongo annotation tree ref scoping", () => {
  ...
  it("declares every prop-level annotation with passedWhenReferred: false", () => {
    const missing = collectPropSpecs(annotations, "mongo")
      .filter(
        ([, spec]) =>
